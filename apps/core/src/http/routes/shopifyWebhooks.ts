...
async function handleCompliancePost(
  ...
  return json({ accepted: true, compliance: true, topic }, 200);
}

export async function postShopifyWebhooks(request: Request): Promise<Response> {
  ...
  if (!connectionId) {
    connectionId =
      (process.env.CHANNELS_SHOPIFY_DEFAULT_CONNECTION_ID || "").trim() || null;
  }
  ...
  if (!secret || !verifyShopifyWebhookHmac({ rawBody, hmacHeader, secret })) {
    console.warn("shopify_webhook_hmac_rejected", { shopDomain, topic });
    return json({ error: "Unauthorized" }, 401);
  }
